...
class VirtualGamepadDevice:
    ...
    def initialize(self) -> bool:
        ...
        if uinput is None:
            self.logger.error(
                "python-uinput not available. Install dependencies with: uv sync"
            )
            return False
        ...
        try:
            ...
            self.logger.error(
                "Make sure you have proper permissions to create uinput devices."
            )
            self.logger.error(
            ...
            if button_name.startswith("button_"):
                try:
                    ...
                    if 0 <= button_num < len(self.button_map):
                        self.device.emit(
                            self.button_map[button_num], 1 if is_pressed else 0
                        )
