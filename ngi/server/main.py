...
class GamepadServerApp:
    ...
    def _handle_gamepad_state(self, state: GamepadState):
        ...
        if success:
            ...
            if self.messages_received % 60 == 0:  # Every 60 messages (roughly 1 second at 60Hz)
                self.logger.info(f"Messages received: {self.messages_received}")
                self.logger.debug(f"Gamepad state - Steering: {state.steering:.2f}, "
    ...
    async def _status_reporting_task(self):
        ...
        while self.running:
            try:
                ...
                self.logger.info(f"Server status - Connected clients: {len(clients)}, "
