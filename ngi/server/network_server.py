...
class GamepadServer:
    ...
    async def _handle_message(self, websocket, message: NetworkMessage):
        ...
        try:
            if message.type == MessageType.CONNECT:
                ...
                self.logger.info(f"Client {client_address} identified as: {client_type}")
            ...
            self.logger.error(f"Error handling {message.type.value} message from {client_address}: {e}")
    ...
    async def broadcast_force_feedback(self, feedback: ForceFeedback):
        ...
        gamepad_clients = [
            client for client, info in self.client_info.items()
            if info.get("client_type") == "gamepad_client"
        ]
        
        if gamepad_clients:
            self.logger.debug(f"Broadcasting force feedback to {len(gamepad_clients)} clients")
            await asyncio.gather(
