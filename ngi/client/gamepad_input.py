...
class GamepadInputHandler:
    """Handles reading input from gamepad devices, specifically optimized for Logitech G920."""
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not g920_found and pygame.joystick.get_count() > 0:
                ...
                self.joystick.init()
                self.logger.info(f"Using first available device: {self.joystick.get_name()}")
            
            if self.joystick:
                self.logger.info(f"Gamepad initialized: {self.joystick.get_name()}")
                self.logger.info(f"Axes: {self.joystick.get_numaxes()}, Buttons: {self.joystick.get_numbuttons()}")
    ...
    def read_gamepad_state(self) -> Optional[GamepadState]:
        ...
        try:
            ...
            if self.joystick.get_numbuttons() > max(self.g920_mappings['gear_buttons'].values()):
                if self.joystick.get_button(self.g920_mappings['gear_buttons']['reverse']):
                    gear = -1
                else:
                    for gear_num in range(1, 7):
                        gear_key = f'gear_{gear_num}'
                        if (gear_key in self.g920_mappings['gear_buttons'] and 
                            self.joystick.get_button(self.g920_mappings['gear_buttons'][gear_key])):
                            gear = gear_num
                            break
